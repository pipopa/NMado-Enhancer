...
document.querySelector("#save-button").addEventListener("click", async () => {
  ...
  const useHighlight = document
    .querySelector("#use-highlight")
    .classList.contains("is-checked");
  const useNotification = document
    .querySelector("#use-notification")
    .classList.contains("is-checked");
  const hideChat = document
    .querySelector("#hide-chat")
    .classList.contains("is-checked");
...
// Notification Test
document
  .querySelector("#notification-test-button")
  .addEventListener("click", () => {
...
const init = async () => {
  ...
  if (useHighlight) {
    ...
    setTimeout(() => {
      useHighlightCheckBox.MaterialCheckbox.check();
    }, 200);
  }
  ...
  if (useNotification) {
    ...
    setTimeout(() => {
      useNotificationCheckBox.MaterialCheckbox.check();
    }, 200);
  }
  ...
  if (hideChat) {
    ...
    setTimeout(() => {
      hideChatCheckBox.MaterialCheckbox.check();
    }, 200);
  }
};
init();
